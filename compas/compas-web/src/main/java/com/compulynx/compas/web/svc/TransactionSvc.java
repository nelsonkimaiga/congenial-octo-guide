...
public class TransactionSvc {
	...
	//@Autowired
	//UploadSmartTransactionMstBal uploadSmartTransactionMstBal;
...
//		String path = compasProperties.getCompasProperties().settleMentFilePath+file_name;
	...
	private Response download(String filePath) {
        ...
        if (file.exists()) {
            ...
            logger.info(String.format("Inside downloadFile ==> fileName: %s, fileSize: %s bytes", filePath, myFormat.format(file_size)));
        } else {
...
//	@Consumes(MediaType.MULTIPART_FORM_DATA)
//	@Produces(MediaType.APPLICATION_JSON)
//	@Path("/uploadsmartclaim")
//	public Response uploadSmartFile(
//		@FormDataParam("file") InputStream uploadedInputStream, 
//		@FormDataParam("orgId") String orgId,
//		@FormDataParam("file") FormDataContentDisposition fileDetail 
//    )
...
//            PATH = TOMCAT_HOME+"/lct_uploads/claims_uploads/";
//            String uploadedFileLocation = PATH + fileName;
//            System.out.println("TransactionSvc uploaded smart file path: "+uploadedFileLocation);
//            File file = new File(uploadedFileLocation);
//            try
//            {
//            	file.createNewFile();
...
//            // save it
//            if(writeToFile(uploadedInputStream, uploadedFileLocation)){
//                response = uploadSmartTransactionMstBal.UploadService(uploadedFileLocation);
//                return Response.status(200).entity(response).build();
//            } else{
//                return Response.status(200).entity(new CompasResponse(201,"Error uploading file, Please try again")).build();
//            }
