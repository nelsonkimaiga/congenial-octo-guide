 ...
 */
'use strict';
angular.module('app.memberinquiry', []).controller("memberInquiryCtrl", ["$scope", "$filter", "memberInquirySvc","memStatementSvc", "$memberValid", "$rootScope", "blockUI", "logger" ,"$location","memberSvc","organizationSvc", function ($scope, $filter, memberInquirySvc,memStatementSvc, $memberValid, $rootScope, blockUI, logger, $location,memberSvc,organizationSvc) {
    var init;
    $scope.bioView = true;
...
                    {
                   //  console.log("This is the list of member vouchers :"
					// +JSON.stringify(member.programmes[i].membervouchers[y]));
						// looping to services
						for (var j=0; j<((member.programmes[i].membervouchers[y].services).length); j++)
						{
							// console.log("services: "
							// +JSON.stringify(member.programmes[i].membervouchers[y].services[j]));
...
         return $http({
// url:
// 'http://192.168.13.237:8085/lct/akuh/memberVerification/save_verified_lct_member',
