 ...
 */
'use strict';
angular.module('app.rptCashTransaction', []).controller("rptcashTransactionCtrl", ["$scope", "$filter", "memSvc","memberInquirySvc","merchantSvc", "productSvc", "organizationSvc","agentSvc","deviceSvc","voucherTopupSvc", "$memValid", "$rootScope", "blockUI", "logger", "$location","$http","$window", function ($scope, $filter, memSvc,memberInquirySvc,merchantSvc, productSvc,organizationSvc,agentSvc,deviceSvc,voucherTopupSvc,$memValid, $rootScope, blockUI, logger, $location,$http,$window) {
	$scope.mem=[];
    ...
    $scope.exportReport=function(type)    {
        ...
        if(FromDt == null){
            ...
            $scope.url = '/compas/reports?type=OB&FrDt=' +FromDt+'&ToDt='+ToDt+'&orgId='+orgId+'&eType='+type;
        }
...
$scope.loadTransactions=function(){
  ...
  memSvc.GetOutstandingDetailedTxn(gate).success(function (response) {
...
//$scope.loadTransactions();
    ...
    var shpMemStatement = {};
    shpMemStatement.GetOutstandingDetailedTxn = function (gate) {
        return $http({
            url: '/compas/rest/transaction/gtOutstandingTxnDetail/',
            method: 'POST',
