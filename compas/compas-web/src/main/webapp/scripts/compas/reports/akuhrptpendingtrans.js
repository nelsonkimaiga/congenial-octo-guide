 ...
 */
'use strict';
angular.module('app.akuhrptPendingTransaction', []).controller("akuhrptpendingTransactionCtrl", ["$scope", "$filter", "pendingTransactionsSvc","memberInquirySvc", "merchantSvc", "productSvc","organizationSvc","agentSvc","deviceSvc","voucherTopupSvc", "$memValid", "$rootScope", "blockUI", "logger", "$location","$http","$window", function ($scope, $filter, pendingTransactionsSvc,memberInquirySvc,merchantSvc, productSvc,organizationSvc,agentSvc,deviceSvc,voucherTopupSvc,$memValid, $rootScope, blockUI, logger, $location,$http,$window) {
	$scope.mem=[];
    ...
    $scope.exportReport=function(type)    {
        ...
        if(FromDt == null){
		   ...
		   return false;
            ...
            $scope.url = '/compas/reports?type=PB&FrDt=' +FromDt+'&ToDt='+ToDt+'&orgId='+orgId+'&eType='+type;
        }
...
$scope.loadTransactions=function(){
  ...
  pendingTransactionsSvc.GetPendingDetailedTxn(gate).success(function (response) {
...
}]).factory('pendingTransactionsSvc', function ($http) {
    var pendingTransactions = {};
    pendingTransactions.GetPendingDetailedTxn = function (gate) {
        return $http({
            url: '/compas/rest/transaction/gtPendingTxnDetail/',
            method: 'POST',
