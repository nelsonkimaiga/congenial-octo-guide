 ...
 */
'use strict';
angular.module('app.akuhrptRejectedTransaction', []).controller("akuhrptrejectedTransactionCtrl", ["$scope", "$filter", "rejectedTransactionsSvc", "memberInquirySvc", "merchantSvc", "productSvc", "organizationSvc", "agentSvc", "deviceSvc", "voucherTopupSvc", "$memValid", "$rootScope", "blockUI", "logger", "$location", "$http", "$window", function ($scope, $filter, rejectedTransactionsSvc, memberInquirySvc, merchantSvc, productSvc, organizationSvc, agentSvc, deviceSvc, voucherTopupSvc, $memValid, $rootScope, blockUI, logger, $location, $http, $window) {
	$scope.mem = [];
	...
	$scope.exportReport = function (type) {
		...
		if (FromDt == null) {
			...
			$scope.url = '/compas/reports?type=RB&FrDt=' + FromDt + '&ToDt=' + ToDt + '&orgId=' + orgId + '&eType=' + type;
		}
	...
	$scope.loadTransactions = function () {
		...
		rejectedTransactionsSvc.GetRejectedDetailedTxn(gate).success(function (response) {
...
}]).factory('rejectedTransactionsSvc', function ($http) {
	var rejectedTransactions = {};
	rejectedTransactions.GetRejectedDetailedTxn = function (gate) {
		return $http({
			url: '/compas/rest/transaction/gtRejectedTxnDetail/',
			method: 'POST',
			headers: {
