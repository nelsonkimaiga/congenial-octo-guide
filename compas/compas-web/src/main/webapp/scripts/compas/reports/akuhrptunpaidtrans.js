 ...
 */
'use strict';
angular.module('app.akuhrptUnpaidTransaction', []).controller("akuhrptunpaidTransactionCtrl", ["$scope", "$filter", "unpaidTransactionsSvc", "memberInquirySvc", "merchantSvc", "productSvc", "organizationSvc", "agentSvc", "deviceSvc", "voucherTopupSvc", "$memValid", "$rootScope", "blockUI", "logger", "$location", "$http", "$window", function ($scope, $filter, unpaidTransactionsSvc, memberInquirySvc, merchantSvc, productSvc, organizationSvc, agentSvc, deviceSvc, voucherTopupSvc, $memValid, $rootScope, blockUI, logger, $location, $http, $window) {
	$scope.mem = [];
	...
	$scope.exportReport = function (t) {
		...
		if (FromDt == null) {
			...
			$scope.url = '/compas/reports?type=UB&FrDt=' + FromDt + '&ToDt=' + ToDt + '&orgId=' + orgId + '&eType=' + t;
		}
	...
	$scope.loadTransactions = function () {
		...
		unpaidTransactionsSvc.GetUnpaidDetailedTxn(gate).success(function (response) {
...
}]).factory('unpaidTransactionsSvc', function ($http) {
	var pendingTransactions = {};
	pendingTransactions.GetUnpaidDetailedTxn = function (gate) {
		return $http({
			url: '/compas/rest/transaction/gtUnpaidTxnDetail/',
			method: 'POST',
			headers: {
