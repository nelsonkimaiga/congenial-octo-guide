...
//	
   ...
   $scope.exportReport=function(type)    {
       ...
       if(FromDt == null){
		   ...
		   $scope.url = '/compas/reports?type=CLN&FrDt=' +FromDt+'&ToDt='+ToDt+'&orgId='+orgId+'&eType='+type;
           $location.path('/reports/clinix/transaction');
       }
...
// $scope.productSelect = 0;
   ...
   $scope.loadTransactions=function(){
     ...
     clinixgate.fromDate=$scope.fromDt;
     clinixgate.toDate=$scope.toDt;
     clinixgate.orgId = $scope.orgSelect;
// Gertgate.merchantId = $scope.merchantSelect;
// Gertgate.productId = $scope.productSelect;
     voucherSvc.GetAllDetailedClinixTxn(clinixgate).success(function (response) {
 ...
 // tmrAuthData = $timeout($scope.onTimeout,1000);
   $scope.loadAmpuntDetail=function(){
	   voucherSvc.GetCurrentDetailedClinixTxn().success(function (response) {
