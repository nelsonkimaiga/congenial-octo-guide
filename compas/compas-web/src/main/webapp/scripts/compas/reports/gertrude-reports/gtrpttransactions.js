...
angular.module('app.gtrptTransaction', []).controller("GtrptTransactionCtrl", ["$scope", "$filter", "memStatementSvc", "merchantSvc", "productSvc", "$memStatementValid", "$rootScope","voucherSvc", "organizationSvc", "blockUI", "logger", "$location","$http","$window","$timeout", function ($scope, $filter, memStatementSvc,merchantSvc,productSvc, $memStatementValid, $rootScope,voucherSvc, organizationSvc, blockUI, logger, $location,$http,$window,$timeout) {
   ...
   $scope.exportReport=function(type)    {
       ...
       if(FromDt == null){
           ...
           $scope.url = '/compas/reports?type=GRT&FrDt=' +FromDt+'&ToDt='+ToDt+'&orgId='+orgId+'&eType='+type;
           $location.path('/reports/gertrude/transaction');
       }
...
// Gertgate.productId = $scope.productSelect;
     voucherSvc.GetAllDetailedGertTxn(Gertgate).success(function (response) {
 ...
 // tmrAuthData = $timeout($scope.onTimeout,1000);
   $scope.loadAmpuntDetail=function(){
	   voucherSvc.GetAllDetailedGertTxn().success(function (response) {
