...
//	
   ...
   $scope.exportReport=function(type)    {
       ...
       if(FromDt == null){
		   ...
		   $scope.url = '/compas/reports?type=NHT&FrDt=' +FromDt+'&ToDt='+ToDt+'&orgId='+orgId+'&eType='+type;
           $location.path('/reports/nairobihosp/transaction');
       }
...
// Gertgate.productId = $scope.productSelect;
     voucherSvc.GetAllDetailedNairobiHospTxn(Gertgate).success(function (response) {
 ...
 // tmrAuthData = $timeout($scope.onTimeout,1000);
   $scope.loadAmpuntDetail=function(){
	   voucherSvc.GetCurrentDetailedNairobiHospTxn().success(function (response) {
