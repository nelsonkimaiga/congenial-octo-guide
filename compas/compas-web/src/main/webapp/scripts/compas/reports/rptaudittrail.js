...
angular.module('app.rptAuditTrail', []).controller("rptAuditLogCtrl", ["$scope", "$filter", "memStatementSvc", "merchantSvc", "productSvc", "$memStatementValid", "$rootScope","voucherSvc", "organizationSvc", "blockUI", "logger", "$location","$http","$window","$timeout", function ($scope, $filter, memStatementSvc,merchantSvc,productSvc, $memStatementValid, $rootScope,voucherSvc, organizationSvc, blockUI, logger, $location,$http,$window,$timeout) {
	...
	$scope.exportReport=function(type)    {
		...
		if(FromDt == null){
			...
			$scope.url = '/compas/reports?type=ATR&FrDt=' +FromDt+'&ToDt='+ToDt+'&orgId='+orgId+'&eType='+type;
			$location.path('/reports/audit/auditlog');
		}
...
//		gate.productId = $scope.productSelect;
		voucherSvc.GetAllDetailedAuditLog(gate).success(function (response) {
			$scope.showTransactions=true;
	...
	$scope.loadAmpuntDetail=function(){
		voucherSvc.GetCurrentDetailedAuditLog().success(function (response) {
