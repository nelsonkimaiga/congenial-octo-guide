...
angular.module('app.rptTransaction', []).controller("rptTransactionCtrl", ["$scope", "$filter", "memStatementSvc", "merchantSvc", "productSvc", "$memStatementValid", "$rootScope", "voucherSvc", "organizationSvc", "blockUI", "logger", "$location", "$http", "$window", "$timeout", "localStorageService", function($scope, $filter, memStatementSvc, merchantSvc, productSvc, $memStatementValid, $rootScope, voucherSvc, organizationSvc, blockUI, logger, $location, $http, $window, $timeout, localStorageService) {
    ...
    $scope.loadTransactions = function() {
        ...
        if (LoggedInAdmin === 1 || LoggedInAdmin === 19) {
        	gate.merchantName = current;
        voucherSvc.GetAllDetailedTxn(gate).success(function(response) {
    	 ...
    	 gate.merchantName = current;
    	 console.log("Logging Gate:" , gate);
         voucherSvc.GetAllDetailedServiceProvidersTxn(gate).success(function(response) {
             $scope.showTransactions = true;
             if (response.length > 0) {
                 $scope.ready = true;
             }
    ...
    $scope.loadAmpuntDetail = function() {
    	if (LoggedInAdmin === 1 || LoggedInAdmin === 19) {            
           voucherSvc.GetCurrentDetailedTxn().success(function(response) {
