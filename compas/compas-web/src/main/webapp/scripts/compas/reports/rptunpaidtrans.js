 ...
 */
'use strict';
angular.module('app.rptUnpaidTransaction', []).controller("rptunpaidTransactionCtrl", ["$scope", "$filter", "unpaidTransactionsSvc","memberInquirySvc", "merchantSvc", "productSvc","organizationSvc","agentSvc","deviceSvc","voucherTopupSvc", "$memValid", "$rootScope", "blockUI", "logger", "$location","$http","$window", function ($scope, $filter, unpaidTransactionsSvc,memberInquirySvc, merchantSvc, productSvc,organizationSvc,agentSvc,deviceSvc,voucherTopupSvc,$memValid, $rootScope, blockUI, logger, $location,$http,$window) {
	$scope.mem=[];
