...
//	
   ...
   $scope.exportReport=function(type)    {
       ...
       if(FromDt == null){
		   ...
		   $scope.url = '/compas/reports?type=UHM&FrDt=' +FromDt+'&ToDt='+ToDt+'&orgId='+orgId+'&eType='+type;
           $location.path('/reports/uhmc/transaction');
       }
...
// $scope.productSelect = 0;
   ...
   $scope.loadTransactions=function(){
     ...
     uhmcgate.fromDate=$scope.fromDt;
     uhmcgate.toDate=$scope.toDt;
     uhmcgate.orgId = $scope.orgSelect;
// Gertgate.merchantId = $scope.merchantSelect;
// Gertgate.productId = $scope.productSelect;
     voucherSvc.GetAllDetailedUHMCHospTxn(uhmcgate).success(function (response) {
 ...
 // tmrAuthData = $timeout($scope.onTimeout,1000);
   $scope.loadAmpuntDetail=function(){
	   voucherSvc.GetCurrentDetailedUHMCTxn().success(function (response) {
